...
---
When you ran kubeadm join on master2, the process failed at the check-etcd phase with a context deadline exceeded error. This happened because master2 needed to reach master1's etcd over the network on port 2380 (peer sync) and 2379 (client), but your firewall had neither of those ports open. The join died midway and left master2 in a broken half-joined state — certificates written, but kubelet config never created.
Because /var/lib/kubelet/config.yaml was never written, kubelet had nothing to load on startup and crash-looped over 340 times. That file is generated by kubeadm join at a later phase — since the join never reached that phase, the file simply doesn't exist. The fix is not to create it manually but to reset and re-join cleanly after fixing the firewall.
---

	→ etcd on master1 bound to public IP
	nc (netcat)  tests showed 2380 was refusing on private IP. We checked where etcd was actually listening and found it was on the public interface only. Had to fully reset master1 and re-init with correct private IP.
	                                     Netcat is a network debugging tool used to test if a port is open and reachable.
	Example:  nc -vz 172.168.50.112 6443 
	# Diagnosed with
	sudo ss -tlnp | grep 2380
nc -zv 172.168.50.130 2380
